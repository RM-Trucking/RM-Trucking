...
export default function CustomerViewDetails() {
  ...
  const handleBack = () => {
    navigate(PATH_DASHBOARD?.maintenance?.rateMaintenance?.root);
  }
  // route back if it reloads
  useEffect(() => {
    if(location?.pathname === '/app/maintenance/rate-maintenance/rate-view' && Object.keys(selectedCurrentRateRow).length === 0){
      navigate(PATH_DASHBOARD?.maintenance?.rateMaintenance?.root);
    }
  }, [location]);
