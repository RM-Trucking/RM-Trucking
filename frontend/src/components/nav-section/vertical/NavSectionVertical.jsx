...
export default function NavSectionVertical({ data, ...other }) {
  ...
  const handleCollapseClick = (length,path) => {
    navigate(path);
      ...
      {data.map((group) => {
        ...
        return (
          <List key={key} disablePadding sx={{ px: 2, pt: 1, }}>
            {group?.title && <StyledSubheader disableSticky active={group?.path?.includes(pathname)} onClick={() => handleCollapseClick(group?.children?.length, group?.path)}>{group?.title}</StyledSubheader>}
