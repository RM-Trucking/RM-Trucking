...
const DelCarrierStepper = ({ location, navigate, watchedCarrierInfoSubmit,
    PATH_DASHBOARD,
    setHandleCancelModal,
    hasInitialData,
    handleNext,
    onFormSubmit,
    handleDelCarrierEditNext,
    isPickupPending,
    isSubmitting,
    isSubmittingFinal,
    type,
    ...
    useEffect(() => {
        if (watchedCarrierInfoSubmit && type === 'Edit') {
            handleDelCarrierEditNext(dispatch, setValue, getValues, trigger, errors, activeStep, watchedServiceLevel, watchedAirportPickupService,
                ...
                watchedDestinationAirport, setActiveStep, totals, watchedLinehaulAddAcc, type, selectedDelRowObj, patchNetworkShipment,
            );
        }
            ...
            >
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, gap: 2 }}>
                    ...
                    <Box display={'flex'} alignItems={'center'}>
                        ...
                        <Typography variant="subtitle2" fontWeight="bold">Select Del Agent</Typography>
                    </Box>
                    ...
                    <Box sx={{ flex: '0 1 300px', bgcolor: '#cdcdcd', p: 1, borderRadius: '8px' }}>
                        <Stack spacing={0.5}>
                            <Box sx={{ display: 'flex', borderBottom: '1px solid #ccc', pb: 0.5 }}>
                                ...
                                <Typography sx={valueStyle}>{selectedDelRowObj?.shipmentDetails?.shipmentProNumber}</Typography>
                            </Box>
                            ...
                            <Box sx={{ display: 'flex' }}>
                                ...
                                <Typography sx={valueStyle}>{watch('shipmentType')?.replace(/_/g, ' ')}</Typography>
                            </Box>
                        </Stack>
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
                        ...
                        <Box sx={{ bgcolor: '#bdbdbd', borderRadius: '8px', p: 1, minWidth: '250px' }}>
                            <Box sx={{ display: 'flex', borderBottom: '1px solid #999', pb: 0.5, mb: 0.5 }}>
                                ...
                                <Typography sx={{ ...valueStyle, textAlign: 'right' }}>{watch('serviceLevel')}</Typography>
                            </Box>

                            <Box sx={{ display: 'flex' }}>
                                <Typography sx={{ ...labelStyle, flex: 1 }}>Date Specific :</Typography>
                                {/* Format the Dayjs object to a clean string before rendering */}
                                <Typography sx={{ ...valueStyle, textAlign: 'right' }} >
                                    {watch('date') && dayjs(watch('date')).isValid()
...
export default DelCarrierStepper; 